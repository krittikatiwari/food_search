...
export default function Vegetarian(){
    ...
    useEffect(()=>{
        fetch("http://localhost:2000/home?country=India&foodtype=Vegetarian").then(res => res.json())
        .then((data)=>{
            setAdvice(data)
        }).catch((err)=>{
                 ...
                 advice.map((eac,index)=>{
                    return(
                        <div key={index} className="vegetarian-box"  >
                            <img className="img" src={eac.image} alt="image" />
                            <div className="vegetarian-names"><h2 className="vegetarian-name">{eac.name}</h2></div>
                        </div>
                    )
                    })
            }
